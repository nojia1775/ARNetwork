...
class	ARNetwork
{
	...
	public:
		...
		void					set_loss_function(const std::string& loss) { _loss_function = loss; }
		void					set_layer_function(const std::string& layer) { _layer_function = layer; }
		void					set_output_function(const std::string& output) { _output_function = output; }

		Vector<double>				feed_forward(const Vector<double>& inputs);
		void					back_propagation(std::vector<Matrix<double>>& dW,
							std::vector<Matrix<double>>& dZ, const Vector<double>& y);
		model_measures_type			train(const std::pair<batch_type, batch_type>& inputs,
							const std::pair<batch_type, batch_type>& outputs, const size_t& epochs);
